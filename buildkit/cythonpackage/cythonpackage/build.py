...
def _compile_packages(conf: Dict[str, Any], packages: List[str]) -> List[Extension]:
    ...
    exclude_files = set(
        itertools.chain.from_iterable([glob(g) for g in conf["exclude"]])
    )
...
class _build_py(original_build_py):
    ...
    def finalize_options(self):
        ...
        if os.environ.get("CYTHONPACKAGE_DEBUG", "False").lower() == "true":
            print(
                f"=====> {self.compile=} {self.optimize=} {self.remove_source=} {self.inject_init=} {self._exclude=}"
            )
    ...
    def build_module(self, module, module_file, package) -> Tuple[str, int]:
        ...
        if self.inject_init:
            if outfile.endswith("__init__.py"):
                ...
                if (
                    outfile.endswith("__init__.py")
                    and Path(package_file, "__compile__.py").exists()
                ):
...
def build_cythonpackage(
    setup: Dict[str, Any], conf: Union[bool, Dict[str, Any]] = True
):
